function h = plotSTFT(t,f,X,fs,bLog,DRdB,titleStr)
...
XdB = 20 * log10(abs(X));   % convert to dB magnitude
XdB = XdB - max(XdB(:));          % normalize so max = 0 dB
range = [-DRdB 0];                % dynamic range

disp('Dynamic Range STFT')
disp(range)
